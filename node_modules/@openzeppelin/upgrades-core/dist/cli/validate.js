...
Options:
  ...
  --unsafeAllow "<VALIDATION_ERROR>[,<VALIDATION_ERROR>...]"  Selectively disable one or more validation errors. Comma-separated list with one or more of the following: ${run_1.errorKinds.join(', ')}
...
async function main(args) {
    ...
    if (!help(parsedArgs, extraArgs)) {
        ...
        const result = await (0, _1.validateUpgradeSafety)(functionArgs.buildInfoDir, functionArgs.contract, functionArgs.reference, functionArgs.opts, functionArgs.referenceBuildInfoDirs, functionArgs.exclude);
...
function getAndValidateStringArray(parsedArgs, option, useCommaDelimiter) {
    const value = parsedArgs[option];
    if (value !== undefined) {
...
function assertNonEmptyString(value, option) {
...
function getUnsafeAllowKinds(parseArgs) {
    const unsafeAllowTokens = getAndValidateStringArray(parseArgs, 'unsafeAllow', true) ?? [];
    if (unsafeAllowTokens.some(token => !run_1.errorKinds.includes(token))) {
        throw new Error(`Invalid option: --unsafeAllow "${parseArgs['unsafeAllow']}". Supported values for the --unsafeAllow option are: ${run_1.errorKinds.join(', ')}`);
    }
