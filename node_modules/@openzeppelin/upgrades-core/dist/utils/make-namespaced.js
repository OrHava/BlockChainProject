 ...
 */
function makeNamespacedInput(input, output, solcVersion) {
 ...
 */
function tryRemoveNonStructNatSpec(origContent, solcVersion) {
    const natSpecRemovals = [];
    if (solcVersion !== undefined && tryRequire('@nomicfoundation/slang') && slangSupportsVersion(solcVersion)) {
        ...
        while (cursor.goToNextTerminalWithKinds([TerminalKind.MultiLineNatSpecComment, TerminalKind.SingleLineNatSpecComment])) {
            ...
            (0, assert_1.assert)(triviaNode instanceof TerminalNode);
...
function slangSupportsVersion(solcVersion) {
    /* eslint-disable @typescript-eslint/no-var-requires */
    const { Language } = require('@nomicfoundation/slang/language');
    /* eslint-enable @typescript-eslint/no-var-requires */
...
function replaceFunction(node, orig, modifications) {
    switch (node.kind) {
        case 'freeFunction':
        case 'function': {
            ...
            if (node.returnParameters.parameters.length > 0) {
                modifications.push(makeReplace(node.returnParameters, orig, `(${node.returnParameters.parameters.map(param => toReturnParameterReplacement(param)).join(', ')})`));
            }
            if (node.body) {
        ...
        default:
            return (0, assert_1.assertUnreachable)(node.kind);
    }
}
function toReturnParameterReplacement(param) {
...
function makeInsertBefore(node, text) {
...
function makeDeleteRange(start, end) {
    return { start, end };
}
