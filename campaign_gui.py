...
class CampaignApp:
    ...
    def setup_web3(self):
       ...
       self.account = "0x1e55dC69F540784698Bf43fAA8c0De77A05aaeBc"
       self.private_key = "0x22d18bc60d6d4a077bc61eb90372b90ac56dc7029d33874ad26faed96adfc6d2"  # Add '0x' prefix
    ...
    async def create_campaign_async(self):
        ...
        try:
            ...
            estimated_gas = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.factory_contract.functions.createCampaign(title, goal, ipfs_hash).estimate_gas({'from': self.account})
            )
        
            # Add a buffer to the estimated gas
            gas_limit = int(estimated_gas * 1.2)
        
            # Get current gas price and add a small premium
            ...
            if tx_receipt['status'] == 1:
                ...
                raise Exception("Transaction failed. Check Etherscan for more details.")
    ...
    def donate_to_campaign(self):
        ...
        try:
            campaign_contract = self.web3.eth.contract(address=campaign_address, abi=self.campaign_abi)
            nonce = self.web3.eth.get_transaction_count(self.account)
            transaction = campaign_contract.functions.donate().build_transaction({
                'from': self.account,
                'value': amount,
                'nonce': nonce,
            })
